...
const followUpTask = 'Follow Up Household Member';
const followUpForm = 'cholera_suspicion_follow_up';
harness.userRoles = ['chp'];
...
it('should not trigger if no reports exist', async () => {
    const tasks = await harness.getTasks({title: followUpTask});
...
it('should create and resolve follow up task for household member with case definition', async() =>{
    ...
    let tasks = await harness.getTasks({title: followUpTask});
    expect(tasks.length).to.equal(1);
    let taskSummary =  await harness.countTaskDocsByState({title: followUpTask});
    ...
    const filledFollowUpForm = await harness.loadAction(tasks[0], ['yes', 'no']);
    expect(filledFollowUpForm.errors).to.be.empty;
    tasks = await harness.getTasks({title: followUpTask});
    expect(tasks.length).to.equal(0);
    taskSummary = await harness.countTaskDocsByState({title: followUpTask});
    expect(taskSummary).to.nested.include({
      Completed: 1,
      Failed: 0,
      Draft: 0,
