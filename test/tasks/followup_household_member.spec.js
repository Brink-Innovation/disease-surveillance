...
const {houseHoldAssessmentScenarios, followUpMemberScenarios} = require('../forms/form_inputs')
...
it('should not trigger if no reports exist', async () => {
    const tasks = await harness.getTasks({title: TASKS_TITLE.followUpHouseholdMember});
...
it('should create and resolve follow up task for household member with case definition', async() =>{
    ...
    let tasks = await harness.getTasks({title: TASKS_TITLE.followUpHouseholdMember});
    expect(tasks.length).to.equal(1);
    let taskSummary =  await harness.countTaskDocsByState({title: TASKS_TITLE.followUpHouseholdMember});
    ...
    const filledFollowUpForm = await harness.loadAction(tasks[0], [...Object.values(followUpMemberScenarios.memberVisitedFacility)]);
    expect(filledFollowUpForm.errors).to.be.empty;
    tasks = await harness.getTasks({title: TASKS_TITLE.followUpHouseholdMember});
    expect(tasks.length).to.equal(0);
    taskSummary = await harness.countTaskDocsByState({title: TASKS_TITLE.followUpHouseholdMember});
    expect(taskSummary).to.nested.include({
      Completed: 1,
      Failed: 0,
      Draft: 0,
